...
const errors = ref({}) as { email?: string }

const newWebsite = ref('')
const addWebsite = async () => {
  if (!newWebsite.value) return;

  try {
    isLoading.value = true;
    await axios.post('/api/blocked-websites', {
       name: newWebsite.value,
       url: newWebsite.value
    });
  ...
  <div class="flex flex-col gap-2">
    ...
    <div class="grid gap-2 text-white">
      <div class="relative w-full max-w-sm items-center">
        <Input type="text" placeholder="Youtube" class="pl-10 h-9 text-black rounded-lg" />
    ...
    <div class="grid gap-2 text-white">
      <div class="relative w-full max-w-sm items-center">
        <Input id="search" type="text" placeholder="youtube.com" class="text-black pl-10 h-9 rounded-lg" />
        <span class="absolute start-0 inset-y-0 flex items-center justify-center px-2">
